...
module.exports = function (paper) {
    if (paper.PaperScript) {
        ...
        require.extensions['.pjs'] = function (module, filename) {
            ...
            module.exports = function (canvas) {
                var source = fs.readFileSync(filename, 'utf8'), code = sourceMapSupport + source, compiled = paper.PaperScript.compile(code, {
                    url: filename,
                    source: source,
                    sourceMaps: true,
    ...
    paper.CanvasView.inject({
        ...
        exportFrames: function (options) {
            ...
            var view = this, count = 0, frameDuration = 1 / options.fps, startTime = Date.now(), lastTime = startTime, padding = options.padding || ((options.amount - 1) + '').length, paddedStr = Array(padding + 1).join('0');
            ...
            function exportFrame() {
                ...
                var file = path.join(options.directory, options.prefix + (paddedStr + count).slice(-padding)
                    + '.' + options.format);
        ...
        exportImage: function (path, callback) {
            this.update();
            var out = fs.createWriteStream(path), format = /\.jp(e?)g$/.test(path) ? 'jpeg' : 'png', stream = this._element[format + 'Stream']();
            stream.pipe(out);
            if (callback) {
